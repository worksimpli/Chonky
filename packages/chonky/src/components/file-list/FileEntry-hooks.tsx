...
export const useFileNameComponent = (file: Nullable<FileData>) => {
    return useMemo(() => {
        ...
        let name;
        let extension = null;
        let fileNameWithoutExtn="";

        const isDir = FileHelper.isDirectory(file);
        if (isDir) {
            name = file.name;
            fileNameWithoutExtn= file.name;
        } else {
            const f = file as FileData;
            extension = f.ext ?? _extname(f.name);
            name = f.name.substring(0, f.name.length - extension.length);
            fileNameWithoutExtn = f.name.lastIndexOf(".") === -1 ? f.name : f.name.substring(0,f.name.lastIndexOf("."));
        }
        ...
        return (
            <div className="chonky-file-name" data-chonky-file-id={file?.id ? file.id: ''}>
              <span className="file-name" data-chonky-file-id={file?.id ? file.id: ''}>{file.sortName ?? name}</span>
                ...
                <span className="chonky-file-title" data-chonky-file-id={file?.id ? file.id: ''}>{fileNameWithoutExtn}</span>
            </div>
        );
    }, [file]);
};
